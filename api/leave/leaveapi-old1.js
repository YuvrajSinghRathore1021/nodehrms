...
// Sample backend code for handling role-based queries
router.post("/HandleLeavePage", async (req, res) => {
  ...
  if (!decodedUserData || !decodedUserData.id) {
    return res.status(200).json({ status: false, error: "Missing user id" });
  }
  const { id, role } = decodedUserData;
  let query = "SELECT * FROM leaves WHERE employee_id = ?";
  let queryParams = [id];
  if (role === "admin") {
    query = "SELECT * FROM leaves"; // Admin sees all leaves
    queryParams = [];
  } else if (role === "rm") {
    query =
      "SELECT * FROM leaves WHERE employee_id IN (SELECT id FROM employees WHERE employee_status=1 and status=1 and delete_status=0 and  reporting_manager = ?)";
    queryParams = [id]; // RM sees team members' leaves
  }
  try {
    ...
    res.status(200).json({ status: true, data: leaveResults });
  } catch (error) {
    res.status(200).json({
      status: false,
...
router.post("/LeaveApprove", async (req, res) => {
  ...
  try {
    ...
    if (RmIdValue === 0 || rm_status === 1 || !rm_id) {
      ...
      return res
        .status(200)
        .json({ status: true, message: adminActionMessage });
    }
