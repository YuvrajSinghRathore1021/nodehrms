...
//           .promise()
//           .query(`SELECT leave_type FROM leave_rules WHERE id IN (?)`, [
//             leaveRuleIds
//           ]);
//         leaveType =
//           leaveResults.length > 0
//             ? leaveResults.map((result) => result.leave_type)
//             : ["Unknown"];
//       }
...
router.get("/rulesfetch", (req, res) => {
  ...
  const query = `SELECT * FROM leave_rules Where company_id=?`;
...
WHERE 
 e.employee_status=1 and e.status=1 and e.delete_status=0 and 
...
router.get("/leavesTypes", (req, res) => {
  ...
  const query = "SELECT leave_type FROM leaves";
  db.query(query, [leave_type], (err, result) => {
    if (err) {
...
router.post('/api/Deleteapi', (req, res) => {
  ...
  db.query('DELETE FROM leave_rules WHERE id = ? AND company_id=?', [id, company_id],
    (err, results) => {
      if (err) {
        return res.status(500).json({ status: false, message: 'Error updating leave.', error: err.message });
      }
      if (results.affectedRows === 0) {
        return res.status(200).json({ status: false, message: 'Not found or no changes made.' });
      }
